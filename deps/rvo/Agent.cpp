...
namespace RVO {
	 ...
	 */
	bool linearProgram1(const Vector<Plane> &planes, size_t planeNo, const Line &line, float radius, const Vector3 &optVelocity, bool directionOpt, Vector3 &result);
	 ...
	 * \return  True if successful.
	 */
	bool linearProgram2(const Vector<Plane> &planes, size_t planeNo, float radius, const Vector3 &optVelocity, bool directionOpt, Vector3 &result);
	 ...
	 */
	size_t linearProgram3(const Vector<Plane> &planes, float radius, const Vector3 &optVelocity, bool directionOpt, Vector3 &result);
	 ...
	 * \param   result     A reference to the result of the linear program.
	 */
	void linearProgram4(const Vector<Plane> &planes, size_t beginPlane, float radius, Vector3 &result);
		...
		for (size_t i = 0; i < agentNeighbors_.size(); ++i) {
			...
			if (distSq > combinedRadiusSq) {
				...
				if (dotProduct < 0.0f && sqr(dotProduct) > combinedRadiusSq * wLengthSq) {
					...
					const float t = (b + sqrtf(sqr(b) - a * c)) / a;
		...
		if (this != agent) {
			...
			if (distSq < rangeSq) {
				if (agentNeighbors_.size() < maxNeighbors_) {
					agentNeighbors_.push_back(make_pair(distSq, agent));
				}
				...
				agentNeighbors_[i] = make_pair(distSq, agent);
	...
	bool linearProgram2(const Vector<Plane> &planes, size_t planeNo, float radius, const Vector3 &optVelocity, bool directionOpt, Vector3 &result)
	{
		...
		if (directionOpt) {
			...
			if (planeOptVelocityLengthSq <= RVO_EPSILON) {
				...
				result = planeCenter + sqrtf(planeRadiusSq / planeOptVelocityLengthSq) * planeOptVelocity;
			}
		}
		else {
			...
			if (absSq(result) > radiusSq) {
				...
				result = planeCenter + sqrtf(planeRadiusSq / planeResultLengthSq) * planeResult;
			}
		}
